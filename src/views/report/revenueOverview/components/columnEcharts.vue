<script setup lang="ts">
  ...
  import { getMonthOrYear } from '@/api/modules/report';
  ...
  const getOptionsData = async () => {
    const result = await getMonthOrYear({ status: value.value });
    // console.log(result);
    result.data.forEach((item) => {
      item.type = 'bar';
      // item.stack = 'Total'; //堆叠样式
...
<template>
  <div>
    <div class="flex-between">
      <div class="text-18px">月度/年度收入统计</div>
      <div>
